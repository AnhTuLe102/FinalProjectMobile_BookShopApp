...
public class ManagerOrderActivity extends AppCompatActivity implements OrderAdapter.OnOrderCancelListener {
    ...
    private void initView() {
        ...
        rcDELIVERING = findViewById(R.id.rcDELIVERING);
        rcCOMPLETED = findViewById(R.id.rcCOMPLETED);
    }
    ...
    private void getListOrderDelivering() {
        orderAPIService = RetrofitClient.getRetrofit().create(OrderAPIService.class);
        orderAPIService.getOrdersByOrderTrackId(4).enqueue(new Callback<List<Order>>() {
            @Override
            public void onResponse(Call<List<Order>> call, Response<List<Order>> response) {
                if (response.isSuccessful()) {
                    orderList = response.body();
                    ...
                    orderAdapter = new OrderAdapter(ManagerOrderActivity.this, orderList, ManagerOrderActivity.this);
                    rcDELIVERING.setLayoutManager(new LinearLayoutManager(ManagerOrderActivity.this, RecyclerView.VERTICAL, false));
    ...
    private void getListOrderCompleted() {
        orderAPIService = RetrofitClient.getRetrofit().create(OrderAPIService.class);
        orderAPIService.getOrdersByOrderTrackId(5).enqueue(new Callback<List<Order>>() {
            @Override
            public void onResponse(Call<List<Order>> call, Response<List<Order>> response) {
                if (response.isSuccessful()) {
                    orderList = response.body();
                    ...
                    orderAdapter = new OrderAdapter(ManagerOrderActivity.this, orderList, ManagerOrderActivity.this);
                    rcCOMPLETED.setLayoutManager(new LinearLayoutManager(ManagerOrderActivity.this, RecyclerView.VERTICAL, false));
